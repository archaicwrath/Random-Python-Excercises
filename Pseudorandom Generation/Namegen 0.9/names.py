...
with open('data/syllables_new.txt', 'r') as syl:
...
def genName(name):
    ...
    for i, c in enumerate(name):
        ...
        for j, s in enumerate(syllables):
            percentChance = random.uniform(0, 1)
            if c == syllables[j] and random.uniform(0, 1) > percentChance:
